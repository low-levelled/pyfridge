...
| `viewer_template.html` + `build_viewer.py` | Self-contained browser viewer of the **exported** CAD assembly (embeds the glTF, so it shows STEP geometry, not the hand-built model): STEP part tree with part numbers and C/D/P tags, closed/open, ghost shell, layer toggles, click-to-highlight. `python build_viewer.py` → `out/pyfridge_cad_assembly.html` (~8 MB, not committed). |
